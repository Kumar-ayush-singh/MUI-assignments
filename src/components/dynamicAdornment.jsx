...
function getCountryName(code){
  ...
  const reasonName = new Intl.DisplayNames( ['en'], {type: 'region'});
  return reasonName.of(code);
}
...
export default function DynamicAdornment() {
  const theme = useTheme();

  const [countryCode, setCountryCode] = useState(91);
  const [phoneNumber, setPhoneNumber] = useState('');

  const countryCodeElRef = useRef(null);
  const phoneNumberElRef = useRef(null);

  

  function handleCountryCodeChange(_event){
    setCountryCode(_event.target.value);
    if(phoneNumberElRef.current){
      phoneNumberElRef.current.focus();
      console.log(phoneNumberElRef.current);
    }
  }

  function onDown (){
    if(countryCodeElRef.current){
      countryCodeElRef.current.click();
    }
  }

  function handlePhoneChange(_event){
    let newValue = _event.target.value;
    newValue = newValue.substring(0, 10);
    const numbers = newValue.match(/\d+/);
    setPhoneNumber(numbers ? numbers[0] : '');
  }
            ...
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
                  <Select 
                    ref={countryCodeElRef}
                    renderValue={(P) => `+ ${countryCode}`} 
                    value={countryCode} 
                  ...
                  >
                    {
                      Object.keys(countryDictionary).map((key, i) => {
                        return <MenuItem key={i} value={countryDictionary[key]}>{`${getCountryFlag(key)} ${getCountryName(key)} +${countryDictionary[key]}`}</MenuItem>
                      })
                    }
                  </Select>
                </InputAdornment>
